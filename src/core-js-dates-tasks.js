...
function getNextFriday(date) {
  ...
  const result = date.setDate(date.getDate() + plusDays);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const dayDifference = new Date(dateEnd) - new Date(dateStart);
  const oneDay = 24 * 60 * 60 * 1000;
  return dayDifference / oneDay + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let numOfWeekends = 0;
  for (let i = firstDay; i <= lastDay; i.setUTCDate(i.getUTCDate() + 1)) {
...
function getWeekNumberByDate(date) {
  ...
  const days = (date - startYear) / millisecondsInDay;
